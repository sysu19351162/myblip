...
class BLIP_Decoder_Knowledge(nn.Module):
    def __init__(self,
                 med_config='configs/med_config.json',
                 image_size=384,
                 vit='base',
        ...
        self.knowledge_proj = nn.Linear(self.knowledge_len, vision_width)

        self.prompt = prompt
        self.prompt_length = len(self.tokenizer(self.prompt).input_ids) - 1

    def forward(self, image, caption, knowledge_conceptnet, knowledge_vg):
        ...
        knowledge_output.last_hidden_state = knowledge_output.last_hidden_state.transpose(-1, -2)
        # print(knowledge_output.last_hidden_state.shape)
        knowledge_output.last_hidden_state = self.knowledge_proj(knowledge_output.last_hidden_state).transpose(-1, -2)

        image_embeds = self.visual_encoder(image)
        image_embeds = self.ImageKnowledgeGating(image_embeds, knowledge_output)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
    ...
    def generate(self, image, knowledge_conceptnet, knowledge_vg, sample=False, num_beams=3, max_length=30, min_length=10, top_p=0.9,
...
def blip_decoder_knowledge(pretrained='', **kwargs):
    model = BLIP_Decoder_Knowledge(**kwargs)
    if pretrained:
