...
class BLIP_Pretrain_Knowledge(nn.Module):
    ...
    def forward(self, image, caption, knowledge_conceptnet, knowledge_vg , knowledge_conceptnet_image, knowledge_vg_image ,alpha):
        ...
        image_embeds_origin = image_embeds
        ...
        image_atts_origin = torch.ones(image_embeds_origin.size()[:-1], dtype=torch.long).to(image.device)
        image_feat_origin = F.normalize(self.vision_proj(image_embeds_origin[:, 0, :]), dim=-1)
        ...
        output_pos_origin = self.text_encoder(encoder_input_ids,
                                       attention_mask=text.attention_mask,
                                       encoder_hidden_states=image_embeds_origin,
                                       encoder_attention_mask=image_atts_origin,
                                       return_dict=True,
                                       )
        ...
        image_embeds_neg = []
        image_embeds_neg_origin = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_t2i[b], 1).item()
            image_embeds_neg.append(image_embeds[neg_idx])
            image_embeds_neg_origin.append(image_embeds_origin[neg_idx])

        image_embeds_neg = torch.stack(image_embeds_neg, dim=0)
        image_embeds_neg_origin = torch.stack(image_embeds_neg_origin, dim=0)
        ...
        image_embeds_all_origin = torch.cat([image_embeds_neg_origin, image_embeds_origin], dim=0)
        image_atts_all_origin = torch.cat([image_atts_origin, image_atts], dim=0)
        ...
        output_neg = self.text_encoder(text_ids_all,
                                       attention_mask=text_atts_all,
                                       encoder_hidden_states=image_embeds_all_origin,
                                       encoder_attention_mask=image_atts_all_origin,
                                       return_dict=True,
                                       )

        # output_neg.last_hidden_state = self.TextKnowledgeGating(output_neg.last_hidden_state, knowledge_output_all)

        vl_embeddings_origin = torch.cat([output_pos_origin.last_hidden_state[:, 0, :], output_neg.last_hidden_state[:, 0, :]], dim=0)
        vl_embeddings = torch.cat([output_pos_origin.last_hidden_state[:, 0, :], output_neg.last_hidden_state[:, 0, :]], dim=0)
        vl_output_origin = self.itm_head(vl_embeddings_origin)
        vl_output = self.itm_head(vl_embeddings)

        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long), torch.zeros(2 * bs, dtype=torch.long)],
                               dim=0).to(image.device)
        loss_itm_origin = F.cross_entropy(vl_output_origin, itm_labels)
        loss_itm = F.cross_entropy(vl_output, itm_labels)

        if loss_itm_origin<loss_itm:
            loss_itm = loss_itm_origin
