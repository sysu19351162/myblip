from models.med import BertConfig, ImageKnowledgeGating, TextKnowledgeGating
...
class BLIP_NLVR_Knowledge(nn.Module):
    def __init__(self,
    ...
    def forward(self, image, text, knowledge_conceptnet, knowledge_vg, targets, train=True):
        ...
        image0_embeds = self.ImageKnowledgeGating(image0_embeds, knowledge_output.last_hidden_state)
        image1_embeds = self.ImageKnowledgeGating(image1_embeds, knowledge_output.last_hidden_state)
        ...
        output.last_hidden_state = self.TextKnowledgeGating(output.last_hidden_state, knowledge_output.last_hidden_state)
...
def blip_nlvr_knowledge(pretrained='', **kwargs):
    model = BLIP_NLVR_Knowledge(**kwargs)
    if pretrained:
