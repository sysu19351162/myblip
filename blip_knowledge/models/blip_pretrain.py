...
class BLIP_Pretrain_Knowledge(nn.Module):
    def __init__(self,
                 ...
                 knowledge_ratio=0.3, #使用多少比例的knowledge
                 ):
        """
        Args:
        ...
        self.vg_or_cc = vg_or_cc#True = vg   ;False = cc
    ...
    def forward(self, image, caption, knowledge_conceptnet, knowledge_vg , knowledge_conceptnet_image, knowledge_vg_image ,alpha):
        ...
        # print(image_embeds.shape)#[15, 197, 768])
        ...
        if self.enhance_image:
            ...
            knowledge_output_image = self.text_encoder(knowledge_image.input_ids,
                                                       attention_mask=knowledge_image.attention_mask,
                                                       return_dict=True, mode='text')
            image_embeds = self.text_encoder(encoder_embeds=image_embeds,
                                             attention_mask=torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(
                                                 image.device),
            ...
            if random.random() <self.knowledge_ratio:
                if self.vg_or_cc:
                    caption = knowledge_vg_image
                else:
                    caption = knowledge_conceptnet_image
...
def blip_pretrain_knowledge(pretrained='',**kwargs):
    ...
    print("----not use pretrain parameters----")
    # if pretrained:
