...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    for i, (image, caption, idx, knowledge_conceptnet, knowledge_vg, knowledge_conceptnet_image, knowledge_vg_image) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
...
def evaluation(model, data_loader, device, config):
    ...
    print(text_embeds.shape)#[5070,256]
    ...
    # for image, img_id,knowledge_conceptnet, knowledge_vg in data_loader:
        ...
        # print(image_feat.shape)#([32, 577, 768])
        if config['enhance_image']:
            knowledge = model.tokenizer(knowledge_vg, padding='max_length', truncation=True,
                                    max_length=model.knowledge_len,
                                    return_tensors="pt").to(device)
            knowledge_output_image = model.text_encoder(knowledge.input_ids, attention_mask=knowledge.attention_mask,
                                                        return_dict=True, mode='text')
            image_feat = model.text_encoder(encoder_embeds=image_feat,
                                            attention_mask=torch.ones(image_feat.size()[:-1], dtype=torch.long).to(
                                                image.device),
                                            encoder_hidden_states=knowledge_output_image.last_hidden_state,
                                            return_dict=True,
                                            )
            image_feat = image_feat.last_hidden_state




        # print(knowledge_output_image.last_hidden_state.shape)#([32, 200, 768])
...
def main(args, config):
    ...
    model = blip_retrieval_knowledge(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'],
                             ...
                             queue_size=config['queue_size'], negative_all_rank=config['negative_all_rank'],enhance_image=config['enhance_image'])
